...
def pipeline_locate(args):
    ...
    if einvertedParam is None:
        ...
        }}' {out}.51.outfile | gawk '$4-$3>30 ' >{out}.pos.51.tab
        ...
        }}' {out}.75.outfile | gawk '$4-$3>30 ' >{out}.pos.75.tab
        
        
        gawk 'BEGIN{{OFS="\\t"}}{{print $1,$2,$5,$0}}' {out}.pos.51.tab |sortBed  > {out}.a.bed
        gawk 'BEGIN{{OFS="\\t"}}{{print $1,$2,$5,$0}}' {out}.pos.75.tab |sortBed  > {out}.b.bed
...
def pipeline_align(args):
    ...
    cmd = '''
    gawk -v out={out} 'BEGIN{{OFS="\\t"}}{{print $1"_F",$2,$3,$4,$5"\\n"$1"_R",$2,$3,$4,$5 > out".bed";print $1"_F", $6 "\\n" $1"_R",$6 > out".info" }}'  {out}
    gawk '{{print $1"\\t"$2"\\t"$3"\\t1\\n"$1"\\t"$4"\\t"$5"\\t1\\n"$1"\\t"$2"\\t"$5"\\t-1"}}' {out}.bed |slopBed -b {oversize} -g {out}.info |\
    sortBed|intersectBed -c -f 1 -a - -b {output}.bed |gawk '{{a[$1]+=$4*$5}}END{{for(i in a){{print i"\\t"a[i]}}}}'|sort -k1,1|sed 's/_F\\t/\\tF\\t/;s/_R\\t/\\tR\\t/' >{output}.span.count'''.format(
        out=invertedfile + ".info.tab", output=output, oversize=oversize)
    print("****** NOW RUNNING COMMAND ******: " + cmd)
    print run_cmd(cmd)

    cmd = '''
    cat {output}.bed|gawk 'BEGIN{{OFS="\\t"}}{{print $4,$5,$1}}'|sed 's/_\(.\)$/\\t\\1/g'|gawk '{{if(and(64,$2)){{P=1}}else{{P=2}};print $1"\\t"P"\\t"$3"\\t"$4}}' > {output}.tab
    cut -f 1-3 {output}.tab|sort|uniq -u|fgrep -f - {output}.tab|cut -f 3-4|sort|uniq -c|gawk '{{print $2"\\t"$3"\\t"$1}}' >{output}.pe.count '''.format(
        output=output)
    print("****** NOW RUNNING COMMAND ******: " + cmd)
    print run_cmd(cmd)
